...
object LexiconRef:
  ...
  def parse(value: String): Either[String, LexiconRef] =
    if value.startsWith("#") then definition(value.drop(1)).map(LexiconRef(None, _))
      ...
      parts match
        case Vector(idText) =>
          Nsid.parse(idText).left.map(_.toString).map(id => LexiconRef(Some(id), "main"))
  ...
  private def definition(value: String): Either[String, String] =
    Either.cond(DefinitionName.matches(value), value, "definition name must be 1-64 alphanumeric characters starting with a letter")

/** Data schemas supported by the dependency-free runtime validator. */
enum Schema:
  case BooleanValue(constant: Option[Boolean])
  case IntegerValue(minimum: Option[Long], maximum: Option[Long], allowed: Option[Set[Long]], constant: Option[Long])
  case StringValue(
  ...
  case ObjectValue(properties: Vector[(String, Schema)], required: Set[String], nullable: Set[String])
...
object LexiconDocument:
  final case class Limits(maxSchemaDepth: Int = 64, maxDefinitions: Int = 1024, maxProperties: Int = 4096)
  ...
  def parse(json: Json, limits: Limits = Limits()): Either[LexiconError, LexiconDocument] =
    for
      ...
      definitions <- traverseFields(rawDefinitions) { (name, value) =>
        for
          _ <- LexiconRef.parse(s"#$name").left.map(message => LexiconError(s"$$.defs.$name", message))
      ...
      _ <- primary.headOption match
        ...
        case Some(name) => Left(LexiconError(s"$$.defs.$name", "primary definition must be named main"))
    yield LexiconDocument(id, definitions.toMap)

  private def requireVersion(json: Json): Either[LexiconError, Unit] =
    requiredLong(json, "lexicon", "$").flatMap(value => check(value == 1, "$.lexicon", "only Lexicon version 1 is supported"))

  private def parseDefinition(json: Json, path: String, limits: Limits, depth: Int): Either[LexiconError, Definition] =
    requiredString(json, "type", path).flatMap {
      ...
      case kind @ ("query" | "procedure" | "subscription" | "permission-set") => Right(Definition.Primary(kind))
  ...
  private def parseSchema(json: Json, path: String, limits: Limits, depth: Int): Either[LexiconError, Schema] =
    if depth > limits.maxSchemaDepth then Left(LexiconError(path, s"schema nesting exceeds ${limits.maxSchemaDepth}"))
    else
      requiredString(json, "type", path).flatMap {
        case "boolean" =>
          mutuallyExclusive(json, path).flatMap(_ => optionalBoolean(json, "const", path).map(Schema.BooleanValue.apply))
        case "integer" =>
          for
        ...
        case "string" =>
          for
            ...
            _ <- format.fold[Either[LexiconError, Unit]](Right(())) { value =>
              check(StringFormats.contains(value), s"$path.format", s"unsupported Lexicon string format: $value")
            }
          ...
          yield Schema.StringValue(format, minBytes, maxBytes, minGraphemes, maxGraphemes, allowed, constant)
        case "bytes" =>
          for
        ...
        case "blob" =>
          for
            ...
            _ <- maxSize.fold[Either[LexiconError, Unit]](Right(()))(size => check(size >= 0, s"$path.maxSize", "must be non-negative"))
        ...
        case "object" =>
          for
            ...
            _ <- check(rawProperties.length <= limits.maxProperties, s"$path.properties", s"exceeds ${limits.maxProperties} properties")
            ...
            _ <- check(required.distinct.length == required.length, s"$path.required", "must not contain duplicates")
            _ <- check(nullable.distinct.length == nullable.length, s"$path.nullable", "must not contain duplicates")
        ...
        case "ref" =>
          requiredString(json, "ref", path)
            .flatMap(value => LexiconRef.parse(value).left.map(message => LexiconError(s"$path.ref", message)))
            .map(Schema.Reference.apply)
        case "union" =>
          for
            values <- requiredStringVector(json, "refs", path)
            refs <- traverse(values.zipWithIndex) { (value, index) =>
              LexiconRef.parse(value).left.map(message => LexiconError(s"$path.refs[$index]", message))
            }
            closed <- optionalBoolean(json, "closed", path).map(_.getOrElse(false))
            _ <- check(!closed || refs.nonEmpty, path, "a closed union must contain at least one ref")
  ...
  private def mutuallyExclusive(json: Json, path: String): Either[LexiconError, Unit] =
    for
      ...
      _ <- check(constant.isEmpty || default.isEmpty, path, "const and default are mutually exclusive")
    yield ()

  private def checkMime(value: String, path: String): Either[LexiconError, Unit] =
    val parts = value.split("/", -1)
    val valid = value == "*/*" || (parts.length == 2 && parts(0).nonEmpty && parts(1).nonEmpty &&
      !parts(0).contains('*') && (parts(1) == "*" || !parts(1).contains('*')))
  ...
  private def validateRecordKeyStrategy(value: String, path: String): Either[LexiconError, Unit] =
    value match
      ...
      case literal if literal.startsWith("literal:") =>
        RecordKey.parse(literal.drop("literal:".length)).left.map(error => LexiconError(path, error.toString)).map(_ => ())
      case _ => Left(LexiconError(path, "expected tid, nsid, any, or literal:<record-key>"))

  private def ordered[A](minimum: Option[A], maximum: Option[A], path: String)(using ordering: Ordering[A]): Either[LexiconError, Unit] =
    check(minimum.isEmpty || maximum.isEmpty || ordering.lteq(minimum.get, maximum.get), path, "minimum exceeds maximum")

  private def optionalCount(json: Json, name: String, path: String): Either[LexiconError, Option[Int]] =
    optionalLong(json, name, path).flatMap {
      case Some(value) if value < 0 || value > Int.MaxValue => Left(LexiconError(s"$path.$name", "must be a non-negative 32-bit integer"))
      case value => Right(value.map(_.toInt))
    }

  private def required(json: Json, name: String, path: String): Either[LexiconError, Json] =
    json.field(name).left.map(error => LexiconError(s"$path.$name", error.message))
  ...
  private def requiredString(json: Json, name: String, path: String): Either[LexiconError, String] =
    required(json, name, path).flatMap(_.asString.left.map(error => LexiconError(s"$path.$name", error.message)))

  private def optionalString(json: Json, name: String, path: String): Either[LexiconError, Option[String]] =
    optional(json, name, path).flatMap {
      case None => Right(None)
      case Some(value) => value.asString.left.map(error => LexiconError(s"$path.$name", error.message)).map(Some.apply)
    }

  private def requiredLong(json: Json, name: String, path: String): Either[LexiconError, Long] =
    required(json, name, path).flatMap(_.asLong.left.map(error => LexiconError(s"$path.$name", error.message)))

  private def optionalLong(json: Json, name: String, path: String): Either[LexiconError, Option[Long]] =
    optional(json, name, path).flatMap {
      case None => Right(None)
      case Some(value) => value.asLong.left.map(error => LexiconError(s"$path.$name", error.message)).map(Some.apply)
    }

  private def optionalBoolean(json: Json, name: String, path: String): Either[LexiconError, Option[Boolean]] =
    optional(json, name, path).flatMap {
      case None => Right(None)
      case Some(value) => value.asBoolean.left.map(error => LexiconError(s"$path.$name", error.message)).map(Some.apply)
    }

  private def requiredObject(json: Json, name: String, path: String): Either[LexiconError, Vector[(String, Json)]] =
    required(json, name, path).flatMap(_.asObject.left.map(error => LexiconError(s"$path.$name", error.message)))

  private def requiredStringVector(json: Json, name: String, path: String): Either[LexiconError, Vector[String]] =
    required(json, name, path).flatMap(value => stringVector(value, s"$path.$name"))

  private def optionalStringVector(json: Json, name: String, path: String): Either[LexiconError, Option[Vector[String]]] =
    optional(json, name, path).flatMap {
      case None => Right(None)
      case Some(value) => stringVector(value, s"$path.$name").map(Some.apply)
    }

  private def optionalStringSet(json: Json, name: String, path: String): Either[LexiconError, Option[Set[String]]] =
    optionalStringVector(json, name, path).flatMap {
      case Some(values) if values.distinct.length != values.length => Left(LexiconError(s"$path.$name", "must not contain duplicates"))
      case values => Right(values.map(_.toSet))
    }

  private def optionalLongSet(json: Json, name: String, path: String): Either[LexiconError, Option[Set[Long]]] =
    optional(json, name, path).flatMap {
      case None => Right(None)
      case Some(value) =>
        value.asArray.left.map(error => LexiconError(s"$path.$name", error.message)).flatMap { values =>
          traverse(values.zipWithIndex) { (item, index) =>
            item.asLong.left.map(error => LexiconError(s"$path.$name[$index]", error.message))
          }.flatMap { decoded =>
            check(decoded.distinct.length == decoded.length, s"$path.$name", "must not contain duplicates").map(_ => Some(decoded.toSet))
          }
        }
    }

  private def stringVector(json: Json, path: String): Either[LexiconError, Vector[String]] =
    json.asArray.left.map(error => LexiconError(path, error.message)).flatMap { values =>
      traverse(values.zipWithIndex) { (item, index) =>
        item.asString.left.map(error => LexiconError(s"$path[$index]", error.message))
      }
    }

  private def traverse[A, B](values: Vector[A])(decode: A => Either[LexiconError, B]): Either[LexiconError, Vector[B]] =
    values.foldLeft[Either[LexiconError, Vector[B]]](Right(Vector.empty)) { (result, value) =>
      result.flatMap(decoded => decode(value).map(decoded :+ _))
    }

  private def traverseFields[A](values: Vector[(String, Json)])(decode: (String, Json) => Either[LexiconError, A]): Either[LexiconError, Vector[A]] =
    traverse(values) { case (name, value) => decode(name, value) }

  private def check(condition: Boolean, path: String, message: => String): Either[LexiconError, Unit] =
    Either.cond(condition, (), LexiconError(path, message))
