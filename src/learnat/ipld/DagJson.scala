...
object DagJson:
   ...
   *
   * JSON numbers must be integral signed 64-bit values. Exact single-field `$link` and `$bytes`
   * objects are interpreted as IPLD extension values.
   */
  def decode(json: Json): Either[IpldError, Ipld] = json match
    ...
    case Json.Arr(values) => values
        .foldLeft[Either[IpldError, Vector[Ipld]]](Right(Vector.empty)) { (result, value) =>
          for
            decoded <- result
            item <- decode(value)
          yield decoded :+ item
        }.map(Ipld.List.apply)
    case Json.Obj(Vector(("$link", Json.Str(value)))) => Cid.parse(value).left
        .map(error => IpldError(s"invalid $$link: $error")).map(Ipld.Link.apply)
    ...
    case Json.Obj(fields) => fields
        .foldLeft[Either[IpldError, Vector[(String, Ipld)]]](Right(Vector.empty)) {
  ...
  def encode(value: Ipld): Json = value match
    ...
    case Ipld.Bytes(bytes)     => Json
        .obj("$bytes" -> Json.Str(Base64.getEncoder.encodeToString(bytes.toArray)))
