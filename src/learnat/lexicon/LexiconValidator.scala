...
object LexiconRegistry:
  ...
  def from(documents: Vector[LexiconDocument]): Either[LexiconError, LexiconRegistry] =
    val duplicates = documents.groupMapReduce(_.id)(_ => 1)(_ + _)
      .collect { case (id, count) if count > 1 => id.value }
    if duplicates.nonEmpty then
      Left(LexiconError("$", s"duplicate Lexicon document IDs: ${duplicates.mkString(", ")}"))
...
final class LexiconValidator(registry: LexiconRegistry, maxDepth: Int = 128):
          ...
          case Some(Definition.Record(_, schema)) =>
            for
              ...
              typeName <- fields.get("$type") match
                ...
                case None                  =>
                  Left(ValidationError("$.$type", "record requires a $type discriminator"))
            ...
            yield report
          case Some(_) =>
            Left(ValidationError("$", s"${documentId.value}#main is not a record definition"))
    ...
    if depth > maxDepth then Left(ValidationError(path, s"value nesting exceeds $maxDepth"))
      ...
      schema match
        case Schema.BooleanValue(constant) => value match
            case Ipld.Bool(actual) =>
              constraint(constant.forall(_ == actual), path, "boolean does not match const")
            case _ => wrongType(path, "boolean", value)
        case Schema.IntegerValue(minimum, maximum, allowed, constant) => value match
            case Ipld.Integer(actual) => constraints(
                Vector(
        ...
        case Schema.StringValue(
            ...
            case Ipld.Text(actual) =>
              ...
              for
                report <- constraints(
                  Vector(
                    minBytes.forall(byteLength >= _) -> s"must contain at least ${minBytes
                        .getOrElse(0)} UTF-8 bytes",
                    maxBytes.forall(byteLength <= _) -> s"must contain at most ${maxBytes
                        .getOrElse(0)} UTF-8 bytes",
                    minGraphemes.forall(graphemeLength >= _) ->
                      s"must contain at least ${minGraphemes.getOrElse(0)} graphemes",
                    maxGraphemes.forall(graphemeLength <= _) ->
                      s"must contain at most ${maxGraphemes.getOrElse(0)} graphemes",
                    allowed.forall(_.contains(actual)) -> "is not in the closed enum",
                    constant.forall(_ == actual) -> "does not match const"
                  ),
                  path
                )
                _ <- format.fold[Either[ValidationError, ValidationReport]](Right(
                  ValidationReport.empty
                ))(name => validateFormat(name, actual, path))
              yield report
            case _ => wrongType(path, "string", value)
        case Schema.BytesValue(minimum, maximum) => value match
            case Ipld.Bytes(bytes) => constraints(
                Vector(
                  minimum.forall(bytes.size >= _) -> s"must contain at least ${minimum
                      .getOrElse(0)} bytes",
                  maximum.forall(bytes.size <= _) -> s"must contain at most ${maximum
                      .getOrElse(0)} bytes"
                ),
                path
              )
            ...
            case Ipld.List(values) =>
              for
                lengthReport <- constraints(
                  Vector(
                    minimum.forall(values.length >= _) -> s"must contain at least ${minimum
                        .getOrElse(0)} items",
                    maximum.forall(values.length <= _) -> s"must contain at most ${maximum
                        .getOrElse(0)} items"
                  ),
                  path
                )
        ...
        case Schema.ObjectValue(properties, required, nullable) => asMap(value, path)
            .flatMap { fields =>
              val missing = required.diff(fields.keySet)
              if missing.nonEmpty then
                Left(ValidationError(
                  path,
                  s"missing required properties: ${missing.toVector.sorted.mkString(", ")}"
                ))
              else
                val known = properties.map(_._1).toSet
                val warnings = fields.keysIterator.filterNot(known).filterNot(_ == "$type").toVector
                  .sorted.map(name =>
                    ValidationWarning(
                      s"$path.$name",
                      "unexpected field ignored for forward compatibility"
                    )
                  )
                validateMany(properties) { (name, propertySchema) =>
                  fields.get(name) match
                    ...
                    case Some(Ipld.Null)                            =>
                      Left(ValidationError(s"$path.$name", "null is not allowed"))
                    case Some(propertyValue) => validateSchema(
                        propertySchema,
                        propertyValue,
                        documentId,
                        s"$path.$name",
        ...
        case Schema.Reference(target) => validateReference(
            target,
            value,
            documentId,
            path,
        ...
        case Schema.Union(variants, closed) =>
          validateUnion(variants, closed, value, documentId, path, depth, resolving)
        case Schema.Unknown => value match
            case Ipld.Map(fields) if !fields.exists { case (name, item) =>
                  name == "$type" && item == Ipld.Text("blob")
                } => Right(ValidationReport.empty)
            case Ipld.Map(_) =>
              Left(ValidationError(path, "unknown may not disguise a blob compound value"))
  ...
  private def validateReference(
    ...
    if resolving.contains(key) then Left(ValidationError(path, s"cyclic Lexicon ref: $target"))
      ...
      registry.resolve(documentId, target) match
        ...
        case Some((resolvedId, Definition.Record(_, schema))) =>
          for
            ...
            _ <- fields.get("$type") match
              case Some(Ipld.Text(actual)) => expect(
                  actual == canonicalType(resolvedId, target.definition),
                  s"$path.$$type",
                  s"expected ${canonicalType(resolvedId, target.definition)}"
                )
              case _ => Left(ValidationError(
                  s"$path.$$type",
                  "record reference requires a string discriminator"
                ))
            report <- validateSchema(schema, value, resolvedId, path, depth + 1, resolving + key)
          yield report
        case Some((_, Definition.Token)) =>
          Left(ValidationError(path, s"ref may not target token: $target"))
        case Some((_, Definition.Primary(kind))) =>
          Left(ValidationError(path, s"ref may not target $kind: $target"))
  ...
  private def validateUnion(
    ...
    for
      ...
      typeName <- fields.get("$type") match
        ...
        case Some(_)                 =>
          Left(ValidationError(s"$path.$$type", "union discriminator must be a string"))
        case None =>
          Left(ValidationError(s"$path.$$type", "union value requires a $type discriminator"))
      ...
      report <- selected match
        case Some(reference) => validateReference(
            reference,
            value,
            documentId,
            path,
        ...
        case None if closed =>
          Left(ValidationError(s"$path.$$type", s"unknown closed-union variant: $typeName"))
        case None => Right(ValidationReport(Vector(
            ValidationWarning(s"$path.$$type", s"unknown open-union variant preserved: $typeName")
          )))
    yield report

  private def validateBlob(
      value: Ipld,
    ...
    for
      ...
      _ <- expect(
        accept.isEmpty || accept.exists(mimeMatches(_, mimeType)),
        s"$path.mimeType",
        s"MIME type is not accepted: $mimeType"
      )
      _ <- expect(
        maxSize.forall(size <= _),
        s"$path.size",
        s"blob exceeds maximum size ${maxSize.getOrElse(0L)}"
      )
  ...
  private def validateFormat(
    ...
    val valid = format match
      ...
      case _               => false
    expect(valid, path, s"does not satisfy Lexicon string format $format")
      .map(_ => ValidationReport.empty)
      ...
      case pattern(
        ...
        if offsetText == "-00:00" then false
          ...
          try
            ...
            val nanos = Option(fraction)
              .fold(0)(digits => (digits.take(9) + "000000000").take(9).toInt)
            val offset = if offsetText == "Z" then ZoneOffset.UTC else ZoneOffset.of(offsetText)
            val instant = LocalDateTime.of(year, month, day, hour, minute, second, nanos)
              .toInstant(offset)
  ...
  private def mimeMatches(pattern: String, actual: String): Boolean = pattern == "*/*" ||
    pattern == actual || (pattern.endsWith("/*") && actual.startsWith(pattern.dropRight(1)))

  private def asMap(value: Ipld, path: String): Either[ValidationError, Map[String, Ipld]] =
    value match
      case Ipld.Map(fields) => Right(fields.toMap)
      case _                => wrongType(path, "object", value)

  private def validateMany[A](values: Vector[A])(
      validate: A => Either[ValidationError, ValidationReport]
  ): Either[ValidationError, ValidationReport] = values
    .foldLeft[Either[ValidationError, ValidationReport]](Right(ValidationReport.empty)) {
      (result, value) =>
        for
          accumulated <- result
  ...
  private def constraints(
      checks: Vector[(Boolean, String)],
      path: String
  ): Either[ValidationError, ValidationReport] = checks.collectFirst { case (false, message) =>
    ValidationError(path, message)
  }.toLeft(ValidationReport.empty)

  private def constraint(
      valid: Boolean,
      path: String,
      message: String
  ): Either[ValidationError, ValidationReport] = expect(valid, path, message)
    .map(_ => ValidationReport.empty)
  ...
  private def expect(
      ...
      path: String,
      message: => String
  ): Either[ValidationError, Unit] = Either.cond(valid, (), ValidationError(path, message))

  private def wrongType(
      path: String,
      expected: String,
      actual: Ipld
  ): Left[ValidationError, Nothing] =
