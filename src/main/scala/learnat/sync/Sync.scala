 ...
 *
 * Polling is less efficient than applying firehose diffs, but it is the same
 * recovery operation required after a cursor gap. State changes only after a
 * complete CAR, commit signature, MST, and record verification succeeds.
 */
  ...
  def syncOnce(): Either[SyncError, SyncResult] = synchronized {
    for
      ...
      result <- current match
        case Some(snapshot) if snapshot.commitCid == latest.cid && snapshot.revision == latest.revision =>
          Right(SyncResult.Unchanged(snapshot))
        case previous =>
          for
            car <- client.getRepo(did).left.map(fromClient)
            verified <- RepositoryVerifier.verifyCar(car, did, signingKey).left.map(error => SyncError(error.message))
            _ <- Either.cond(
              !latest.revision.newerThan(verified.commit.rev),
              (),
              SyncError(s"repository export revision ${verified.commit.rev.value} is older than advertised ${latest.revision.value}")
            )
...
object EventStreamCodec:
  ...
  def decode(bytes: Array[Byte]): Either[SyncError, EventFrame] =
    DagCbor.decodeSequence(bytes, DagCbor.Limits(maxBytes = 5 * 1024 * 1024), maxItems = 2)
      .left.map(error => SyncError(error.toString))
      .flatMap {
        case Vector(Ipld.Map(headerFields), body) =>
          val header = headerFields.toMap
          header.get("op") match
            case Some(Ipld.Integer(1)) => header.get("t") match
              case Some(Ipld.Text(eventType)) if eventType.startsWith("#") => Right(EventFrame.Message(eventType, body))
        ...
        case values => Left(SyncError(s"event frame must contain exactly header and body, found ${values.length} values"))
      }

/**
 * JDK WebSocket consumer for a `com.atproto.sync.subscribeRepos` endpoint.
 * Binary fragments are assembled up to the protocol's 5 MiB frame limit and
 * decoded before delivery to the callback.
 */
object FirehoseClient:
  def connect(
      service: URI,
      cursor: Option[Long] = None
  )(onFrame: Either[SyncError, EventFrame] => Unit): Either[SyncError, CompletableFuture[WebSocket]] =
    service.getScheme match
      case "https" | "http" | "wss" | "ws" =>
        val scheme = if service.getScheme == "https" then "wss" else if service.getScheme == "http" then "ws" else service.getScheme
...
private final class FirehoseListener(onFrame: Either[SyncError, EventFrame] => Unit) extends WebSocket.Listener:
  ...
  override def onBinary(webSocket: WebSocket, data: ByteBuffer, last: Boolean): CompletionStage[?] = synchronized {
...
private object FirehoseListener:
  def apply(onFrame: Either[SyncError, EventFrame] => Unit): FirehoseListener = new FirehoseListener(onFrame)
