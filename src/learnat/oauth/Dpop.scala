...
object P256Jwk:
  ...
  def parse(json: Json): Either[OAuthError, P256Jwk] =
    for
      ...
      _ <- Either.cond(
        !values.contains("d"),
        (),
        OAuthError("DPoP jwk must not contain private key material")
      )
  ...
  private def string(fields: Map[String, Json], name: String): Either[OAuthError, String] = fields
    .get(name).toRight(OAuthError(s"DPoP jwk is missing $name"))
    .flatMap(_.asString.left.map(error => OAuthError(s"DPoP jwk $name: ${error.message}")))

/** Claims authenticated by one DPoP proof. */
final case class VerifiedDpopProof(
    jti: String,
    nonce: Option[String],
...
object DpopProof:
  /**
   * Signs one request proof. The access token is included only for protected resource calls; PAR
   * and token endpoint proofs omit `ath`.
   */
  def create(
      keyPair: P256KeyPair,
      method: String,
  ...
  private[oauth] def createWithJti(
    ...
    for
      ...
      _ <- Either.cond(
        jti.nonEmpty && jti.length <= 256,
        (),
        OAuthError("DPoP jti must contain 1-256 characters")
      )
      header = Json.obj(
      ...
      payload = Json.Obj(
        basePayload ++ nonce.map(value => "nonce" -> Json.Str(value)) ++
      ...
      signature <- keyPair.sign(signingInput.getBytes(StandardCharsets.US_ASCII)).left
        .map(error => OAuthError(error.message))
  ...
  private[oauth] def normalizeMethod(method: String): Either[OAuthError, String] =
    ...
    val valid = normalized.nonEmpty &&
      normalized.forall(character => character >= 'A' && character <= 'Z')
  ...
  private[oauth] def normalizeTarget(target: URI): Either[OAuthError, String] =
    ...
    if !valid then
      Left(
        OAuthError("DPoP target must be an absolute HTTP(S) URI without credentials or fragment")
      )
    else
      ...
      try Right(URI.create(s"${scheme.get}://$renderedHost$renderedPort$path").toASCIIString)
        ...
        case error: IllegalArgumentException =>
          Left(OAuthError(s"invalid DPoP target: ${error.getMessage}"))

  private[oauth] def tokenHash(accessToken: String): String = Base64Url.encode(
...
object DpopVerifier:
  /**
   * Verifies structure, public JWK, ES256 signature, request binding, nonce, token hash, issuance
   * time, and optional replay cache in that order.
   */
  def verify(
      proof: String,
    ...
    else if maximumAgeSeconds < 1 || clockSkewSeconds < 0 then
      Left(OAuthError("invalid DPoP verification time policy"))
    else
      proof.split("\\.", -1).toVector match
        case Vector(headerText, payloadText, signatureText) =>
          ...
          for
            ...
            jwkJson <- header.field("jwk").left
              .map(error => OAuthError(s"DPoP header jwk: ${error.message}"))
            ...
            _ <- Either
              .cond(htm == method, (), OAuthError("DPoP htm does not match the HTTP request"))
            htu <- stringField(payload, "htu")
            target <- DpopProof.normalizeTarget(expectedTarget)
            _ <- Either
              .cond(htu == target, (), OAuthError("DPoP htu does not match the HTTP request"))
            issuedAt <- longField(payload, "iat")
            _ <- Either.cond(
              issuedAt <= nowEpochSeconds + clockSkewSeconds,
              (),
              OAuthError("DPoP proof was issued in the future")
            )
            _ <- Either.cond(
              issuedAt >= nowEpochSeconds - maximumAgeSeconds - clockSkewSeconds,
              (),
              OAuthError("DPoP proof is too old")
            )
            nonce <- optionalStringField(payload, "nonce")
            _ <- (nonce, expectedNonce) match
              case (Some(actual), Some(expected)) =>
                constantString(actual, expected, "DPoP nonce does not match")
              case (None, None)    => Right(())
              case (None, Some(_)) => Left(OAuthError("DPoP proof requires the server nonce"))
              case (Some(_), None) =>
                Left(OAuthError("initial DPoP proof must not contain a nonce"))
            ath <- optionalStringField(payload, "ath")
            _ <- accessToken match
              case Some(token) => ath match
                  case Some(actual) => constantString(
                      actual,
                      DpopProof.tokenHash(token),
                      "DPoP access-token hash does not match"
                    )
                  case None => Left(OAuthError("protected-resource DPoP proof requires ath"))
              case None => Either
                  .cond(ath.isEmpty, (), OAuthError("PAR/token DPoP proof must not contain ath"))
  ...
  private def decodeJson(encoded: String, label: String): Either[OAuthError, Json] = Base64Url
    .decode(encoded, label).flatMap { bytes =>
      try
        ...
        case _: java.nio.charset.CharacterCodingException =>
          Left(OAuthError(s"$label is not valid UTF-8"))
    }

  private def expectString(json: Json, name: String, expected: String): Either[OAuthError, Unit] =
    stringField(json, name).flatMap(actual =>
      Either.cond(actual == expected, (), OAuthError(s"DPoP $name must be $expected"))
    )

  private def stringField(json: Json, name: String): Either[OAuthError, String] = json.field(name)
    .flatMap(_.asString).left.map(error => OAuthError(s"DPoP $name: ${error.message}"))
  ...
  private def optionalStringField(json: Json, name: String): Either[OAuthError, Option[String]] =
    json.optionalField(name).left.map(error => OAuthError(error.message)).flatMap {
      ...
      case Some(value) => value.asString.left
          .map(error => OAuthError(s"DPoP $name: ${error.message}")).map(Some.apply)
    }

  private def longField(json: Json, name: String): Either[OAuthError, Long] = json.field(name)
    .flatMap(_.asLong).left.map(error => OAuthError(s"DPoP $name: ${error.message}"))
...
final class DpopReplayCache(maxEntries: Int):
  ...
  def accept(
      ...
      nowEpochSeconds: Long,
      retainSeconds: Long
  ): Either[OAuthError, Unit] = synchronized {
...
final class DpopNonceManager private (
  ...
  def accepts(value: String, nowEpochSeconds: Long): Boolean = synchronized {
    rotateIfNeeded(nowEpochSeconds)
    secureEquals(value, current._1) ||
    previous.exists((nonce, expires) => expires > nowEpochSeconds && secureEquals(value, nonce))
  }

  private def rotateIfNeeded(now: Long): Unit = if now - current._2 >= lifetimeSeconds then
    previous = Some(current._1 -> (now + overlapSeconds))
    current = generate() -> now

  private def secureEquals(left: String, right: String): Boolean = MessageDigest
...
object DpopNonceManager:
  ...
  def secure(
      ...
      random: SecureRandom = SecureRandom()
  ): Either[OAuthError, DpopNonceManager] = create(
    () =>
  ...
  def testing(
    ...
    () =>
      if values.hasNext then values.next()
      else throw IllegalStateException("test nonce sequence exhausted"),
    nowEpochSeconds,
    lifetimeSeconds,
    overlapSeconds
  )
  ...
  private def create(
    ...
    if lifetime < 1 || lifetime > 300 then
      Left(OAuthError("DPoP nonce lifetime must be 1-300 seconds"))
    else if overlap < 0 || overlap >= lifetime then
      Left(OAuthError("DPoP nonce overlap must be non-negative and shorter than its lifetime"))
