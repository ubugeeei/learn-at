...
enum IdentityError:
  ...
  def description: String = this match
    ...
    case ResolutionDisallowed(identifier, reason) =>
      s"resolution of $identifier is disallowed: $reason"
    ...
    case AmbiguousHandle(handle, dids)        =>
      s"handle ${handle.value} resolved to multiple DIDs: ${dids.mkString(", ")}"
    case HandleMismatch(handle, did, claimed) => s"handle ${handle.value} resolved to ${did
          .value}, but the DID document claims ${claimed.fold("no handle")(_.value)}"
...
final class JdkIdentityNetwork private (client: HttpClient) extends IdentityNetwork:
  override def dnsTxt(name: String): Either[IdentityError, Vector[String]] =
    ...
    try
      ...
      case error: Exception                      =>
        Left(IdentityError.Network(s"DNS TXT lookup failed for $name", Some(error)))
  ...
  override def get(uri: URI, maxBytes: Int): Either[IdentityError, IdentityHttpResponse] =
    try
      ...
      if body.length > maxBytes then
        Left(IdentityError.ResponseTooLarge(uri, maxBytes, body.length))
...
final case class DidDocument(
  ...
  def claimedHandle: Option[Handle] = alsoKnownAs.iterator.flatMap { value =>
    if value.startsWith("at://") && !value.drop(5).contains('/') && !value.contains('#') &&
      !value.contains('?')
    then Handle.parse(value.drop(5)).toOption
    else None
  }.nextOption()

  /** Current controlled `#atproto` Multikey string, if present. */
  def atprotoSigningKey: Option[String] = verificationMethods.find { method =>
    matchesFragment(method.id, "#atproto") && method.controller == id.value &&
    method.methodType == "Multikey" && method.publicKeyMultibase.startsWith("z")
  }.map(_.publicKeyMultibase)

  /** Validated origin-level PDS service, with optional local HTTP policy. */
  def pdsEndpoint(allowHttpLocal: Boolean): Option[URI] = services.iterator.filter { service =>
    matchesFragment(service.id, "#atproto_pds") &&
    service.serviceType == "AtprotoPersonalDataServer"
  ...
  private def parsePdsUri(value: String, allowHttpLocal: Boolean): Option[URI] =
    try
      ...
      val local = allowHttpLocal && uri.getScheme == "http" &&
        (uri.getHost == "localhost" || uri.getHost == "127.0.0.1")
...
object DidDocument:
  ...
  def decode(expected: Did, json: Json): Either[IdentityError, DidDocument] =
    for
      ...
      _ <- Either.cond(
        id == expected,
        (),
        invalid(s"id ${id.value} does not match expected ${expected.value}")
      )
  ...
  private def stringArray(json: Json, field: String): Either[IdentityError, Vector[String]] = json
    .optionalField(field).left.map(error => invalid(error.message)).flatMap {
      case None        => Right(Vector.empty)
      case Some(value) => value.asArray.left.map(error => invalid(s"$field: ${error.message}"))
          .flatMap { values =>
            values.foldLeft[Either[IdentityError, Vector[String]]](Right(Vector.empty)) {
              (result, item) =>
                for
                  accumulated <- result
  ...
  private def jsonArray(json: Json, field: String): Either[IdentityError, Vector[Json]] = json
    .optionalField(field).left.map(error => invalid(error.message)).flatMap {
      case None        => Right(Vector.empty)
      case Some(value) => value.asArray.left.map(error => invalid(s"$field: ${error.message}"))
    }

  private def decodeVerificationMethod(json: Json): Option[VerificationMethod] = (for
  ...
  private def decodeService(json: Json): Option[DidService] = (for
    id <- json.field("id").flatMap(_.asString)
...
final class IdentityResolver(
    ...
    config: IdentityResolverConfig = IdentityResolverConfig()
):
  private val disallowedTlds =
    Vector(".alt", ".arpa", ".example", ".internal", ".invalid", ".local", ".localhost", ".onion")
  ...
  def resolveHandle(handle: Handle): Either[IdentityError, Did] = Handle.parse(handle.normalized)
    ...
    .flatMap { normalized =>
      disallowedReason(normalized) match
        ...
        case None         => dnsDid(normalized) match
  ...
  def resolveDid(did: Did): Either[IdentityError, DidDocument] = did.method match
    case "plc" => fetchDidDocument(
        did,
        URI.create(s"${config.plcDirectory.toString.stripSuffix("/")}/${did.value}")
      )
  ...
  private def resolveFromDid(did: Did): Either[IdentityError, ResolvedIdentity] = resolveDid(did)
    .flatMap { document =>
      val verifiedHandle = document.claimedHandle
        .filter(handle => resolveHandle(handle).contains(did))
  ...
  private def assemble(
      ...
      handle: Option[Handle],
      document: DidDocument
  ): Either[IdentityError, ResolvedIdentity] =
    for
  ...
  private def dnsDid(handle: Handle): Either[IdentityError, Option[Did]] = network
    .dnsTxt(s"_atproto.${handle.normalized}").flatMap { records =>
      val dids = records.iterator.filter(_.startsWith("did="))
        .flatMap(value => Did.parse(value.drop(4)).toOption).toVector.distinct
      dids match
  ...
  private def httpsDid(handle: Handle): Either[IdentityError, Did] =
    val developmentHandle = handle.normalized.endsWith(".test") ||
      handle.normalized.endsWith(".localhost")
  ...
  private def fetchDidDocument(did: Did, uri: URI): Either[IdentityError, DidDocument] = fetch(uri)
    .flatMap { response =>
      val text = String(response.body, StandardCharsets.UTF_8)
      Json.parse(text).left.map(error => IdentityError.InvalidDocument(error.toString))
        .flatMap(document => DidDocument.decode(did, document))
    }

  private def fetch(uri: URI): Either[IdentityError, IdentityHttpResponse] = network
    .get(uri, config.maxDocumentBytes).flatMap { response =>
  ...
  private def didWebUri(did: Did): Either[IdentityError, URI] =
    ...
    val decoded =
      try Right(URLDecoder.decode(encoded, StandardCharsets.UTF_8))
        ...
        case _: IllegalArgumentException =>
          Left(IdentityError.ResolutionDisallowed(did.value, "invalid percent encoding"))
    decoded.flatMap { domain =>
      ...
      if domain.contains('/') || (domain.contains(':') && !localhost) then
        Left(IdentityError.ResolutionDisallowed(
          did.value,
          "atproto supports only hostname-level did:web identifiers"
        ))
        ...
        Handle.parse(domain).left
          .map(error => IdentityError.ResolutionDisallowed(did.value, error.message))
          .flatMap { handle =>
  ...
  private def disallowedReason(handle: Handle): Option[String] =
    ...
    disallowedTlds.find(normalized.endsWith).map(tld => s"$tld is not allowed for resolution")
      .orElse(
        if normalized.endsWith(".test") && !config.allowTestTld then
          Some(".test is disabled outside development")
        else None
      )
