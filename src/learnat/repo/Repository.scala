...
object SignedCommit:
  ...
  def unsignedValue(did: Did, data: Cid, rev: Tid, previous: Option[Cid]): Ipld = Ipld.obj(
 ...
 *
 * Mutations return a new instance. This teaching implementation rebuilds the MST from the complete
 * record map to keep the mutation path auditable and to use the deterministic builder as its
 * correctness oracle.
 */
final class Repository private (
  ...
  def get(collection: Nsid, recordKey: RecordKey): Option[RepositoryRecord] = recordsByPath
    .get(Mst.recordPath(collection, recordKey))
  ...
  /**
   * Applies a batch atomically. Any invalid operation leaves this immutable instance usable and no
   * partial commit is produced.
   */
  def applyWrites(writes: Vector[RepositoryWrite]): Either[RepositoryError, Repository] = writes
    .foldLeft[Either[RepositoryError, Map[String, RepositoryRecord]]](Right(recordsByPath)) {
      case (result, RepositoryWrite.Create(record)) => result.flatMap { records =>
          if records.contains(record.path) then
            Left(RepositoryError(s"record already exists: ${record.path}"))
          else validateRecord(record).map(_ => records.updated(record.path, record))
        }
      case (result, RepositoryWrite.Put(record)) => result
          .flatMap(records => validateRecord(record).map(_ => records.updated(record.path, record)))
      case (result, RepositoryWrite.Delete(collection, recordKey)) => result.flatMap { records =>
  ...
  def exportCar: Either[RepositoryError, Array[Byte]] = Car
    .write(CarFile(Vector(commitCid), blocks)).left.map(error => RepositoryError(error.toString))
  ...
  private def validateRecord(record: RepositoryRecord): Either[RepositoryError, Unit] = Repository
    .validateRecord(record)
  private def encodeRecord(record: RepositoryRecord): Either[RepositoryError, (CarBlock, Cid)] =
    Repository.encodeRecord(record)
...
object Repository:
  ...
  def create(
    ...
    val duplicate = initialRecords.groupMapReduce(_.path)(_ => 1)(_ + _)
      .collectFirst { case (path, count) if count > 1 => path }
    duplicate match
      case Some(path) => Left(RepositoryError(s"duplicate initial record: $path"))
      case None       => initialRecords
          .foldLeft[Either[RepositoryError, Map[String, RepositoryRecord]]](Right(Map.empty)) {
            (result, record) =>
              for
                records <- result
            ...
            commit(did, signingKey, tidGenerator, records, previousRevision, None)
          )
  ...
  private[repo] def commit(
    ...
    val encodedRecords = records.values.toVector.sortBy(_.path)
      .foldLeft[Either[RepositoryError, Vector[(String, CarBlock)]]](Right(Vector.empty)) {
        (result, record) =>
          for
            values <- result
    ...
    for
      ...
      mst <- Mst.build(encoded.map { case (path, block) => path -> block.cid }).left
        .map(error => RepositoryError(error.toString))
  ...
  private[repo] def validateRecord(record: RepositoryRecord): Either[RepositoryError, Unit] = Mst
    .validatePath(record.path).left.map(error => RepositoryError(error.toString)).flatMap { _ =>
      record.value match
        case Ipld.Map(fields) => fields.toMap.get("$type") match
            case Some(Ipld.Text(value)) if value == record.collection.value => Right(())
            case Some(Ipld.Text(value))                                     =>
              Left(RepositoryError(s"record ${record.path} has mismatched $$type $value"))
...
final case class VerifiedRepository(
    commitCid: Cid,
    commit: SignedCommit,
    mst: MstSnapshot,
...
object RepositoryVerifier:
  /**
   * Verifies a complete repository export using the expected DID and the signing key obtained from
   * that DID's independently resolved document.
   */
  def verifyCar(
      carBytes: Array[Byte],
      expectedDid: Did,
    ...
    for
      ...
      root <- car.roots match
        ...
        case roots         =>
          Left(RepositoryError(s"repository CAR must have exactly one root, found ${roots.length}"))
      blockMap = car.blocks.map(block => block.cid -> block).toMap
      commitBlock <- blockMap.get(root)
        .toRight(RepositoryError(s"repository CAR is missing root commit block $root"))
      commitValue <- DagCbor.decode(commitBlock.bytes.toArray).left
        .map(error => RepositoryError(error.toString))
      commit <- decodeCommit(commitValue)
      _ <- Either.cond(
        commit.did == expectedDid,
        (),
        RepositoryError(s"commit DID ${commit.did.value} does not match ${expectedDid.value}")
      )
      unsignedBytes <- DagCbor.encode(commit.unsignedValue).left
        .map(error => RepositoryError(error.toString))
      _ <- Either.cond(
      ...
      mst <- MstVerifier.verify(commit.data, car.blocks).left
        .map(error => RepositoryError(error.toString))
      ...
      _ <- Either.cond(
        allowExtraBlocks || extras.isEmpty,
        (),
        RepositoryError(s"repository CAR contains ${extras.size} unreachable blocks")
      )
  ...
  private def decodeCommit(value: Ipld): Either[RepositoryError, SignedCommit] = value match
    case Ipld.Map(fields)
        if fields.map(_._1).toSet == Set("did", "version", "data", "rev", "prev", "sig") =>
      val values = fields.toMap
      for
        did <- values("did") match
          case Ipld.Text(value) => Did.parse(value).left
              .map(error => RepositoryError(error.toString))
          case _ => Left(RepositoryError("commit did must be a string"))
        version <- values("version") match
          case Ipld.Integer(3)     => Right(3L)
          case Ipld.Integer(other) =>
            Left(RepositoryError(s"unsupported repository version $other"))
        ...
        rev <- values("rev") match
          case Ipld.Text(value) => Tid.parse(value).left
              .map(error => RepositoryError(error.toString))
        ...
        signature <- values("sig") match
          ...
          case Ipld.Bytes(bytes)                     =>
            Left(RepositoryError(s"commit signature must be 64 bytes, found ${bytes.size}"))
          case _ => Left(RepositoryError("commit sig must be bytes"))
      yield SignedCommit(did, version, data, rev, previous, signature)
    case Ipld.Map(_) =>
      Left(RepositoryError("commit must contain exactly did, version, data, rev, prev, and sig"))
  ...
  ): Either[RepositoryError, Vector[RepositoryRecord]] = leaves
    .foldLeft[Either[RepositoryError, Vector[RepositoryRecord]]](Right(Vector.empty)) {
      (result, leaf) =>
        for
          records <- result
          block <- blocks.get(leaf.value)
            .toRight(RepositoryError(s"missing record block ${leaf.value} for ${leaf.path}"))
          value <- DagCbor.decode(block.bytes.toArray).left
            .map(error => RepositoryError(s"invalid record ${leaf.path}: $error"))
