...
final case class RecordWriteResult(uri: AtUri, cid: Cid, commitCid: Option[Cid], revision: Option[String])
 ...
 *
 * Construct with a known PDS origin, or use `discover` to resolve an account
 * and bind the client to the PDS declared in its DID document.
 */
final class AtpClient private (val service: URI, private val xrpc: XrpcClient):
  ...
  def login(identifier: AtIdentifier, password: Array[Char]): Either[ClientError, AuthenticatedAtpClient] =
  ...
  def getRecord(repo: AtIdentifier, collection: Nsid, recordKey: RecordKey): Either[ClientError, RecordView] =
    callQuery(
  ...
  def getRepo(did: Did): Either[ClientError, Array[Byte]] =
    nsid("com.atproto.sync.getRepo").flatMap { method =>
  ...
  private[client] def procedure(method: String, input: Json, token: String): Either[ClientError, Json] =
    callProcedure(method, input, Some(token))

  private[client] def query(method: String, parameters: Vector[(String, String)], token: String): Either[ClientError, Json] =
    callQuery(method, parameters, Some(token))

  private def callQuery(method: String, parameters: Vector[(String, String)], token: Option[String]): Either[ClientError, Json] =
    nsid(method).flatMap(value => xrpc.query(value, parameters, token).left.map(fromXrpc))

  private def callProcedure(method: String, input: Json, token: Option[String]): Either[ClientError, Json] =
    nsid(method).flatMap(value => xrpc.procedure(value, input, token).left.map(fromXrpc))

  private def nsid(value: String): Either[ClientError, Nsid] =
  ...
  private def decodeRecordPage(json: Json): Either[ClientError, RecordPage] =
    for
      recordJson <- json.field("records").flatMap(_.asArray).left.map(error => ClientError(error.message))
      records <- recordJson.foldLeft[Either[ClientError, Vector[RecordView]]](Right(Vector.empty)) { (result, item) =>
        for
          values <- result
      ...
      cursor <- json.optionalField("cursor").left.map(error => ClientError(error.message)).flatMap {
        ...
        case Some(value) => value.asString.left.map(error => ClientError(error.message)).map(Some.apply)
      }
  ...
  private def stringField(json: Json, name: String): Either[ClientError, String] =
    json.field(name).flatMap(_.asString).left.map(error => ClientError(error.message))

  private def optionalString(json: Json, name: String): Either[ClientError, Option[String]] =
    json.optionalField(name).left.map(error => ClientError(error.message)).flatMap {
      case None | Some(Json.Null) => Right(None)
      case Some(value) => value.asString.left.map(error => ClientError(error.message)).map(Some.apply)
    }
  ...
  private def optionalCid(json: Json, name: String): Either[ClientError, Option[Cid]] =
    optionalString(json, name).flatMap {
      case None => Right(None)
      case Some(value) => Cid.parse(value).left.map(error => ClientError(error.toString)).map(Some.apply)
    }

object AtpClient:
  /** Creates a client for an already-known XRPC service origin. */
  def create(service: URI, transport: HttpTransport = JdkHttpTransport.default): Either[ClientError, AtpClient] =
    XrpcClient.create(service, transport).left.map(error => ClientError(error.description)).map(new AtpClient(service, _))
  ...
  ): Either[ClientError, (AtpClient, ResolvedIdentity)] =
    resolver.resolve(identifier).left.map(error => ClientError(error.description)).flatMap { identity =>
      create(identity.pds, transport).map(_ -> identity)
    }
...
final case class AuthenticatedAtpClient(client: AtpClient, session: LegacySession):
  ...
  def createRecord(
    ...
    val input = Json.obj(
      ...
      case Json.Obj(fields) => Json.Obj(fields ++ recordKey.map(key => "rkey" -> Json.Str(key.value)))
      case other => other
    client.procedure("com.atproto.repo.createRecord", input, session.accessJwt).flatMap(client.decodeWrite)

  /** Creates or replaces a record at a caller-selected key. */
  def putRecord(collection: Nsid, recordKey: RecordKey, value: Ipld): Either[ClientError, RecordWriteResult] =
    val input = Json.obj(
      "repo" -> Json.Str(session.did.value),
      "collection" -> Json.Str(collection.value),
    ...
    client.procedure("com.atproto.repo.putRecord", input, session.accessJwt).flatMap(client.decodeWrite)
  ...
  def getSession: Either[ClientError, LegacySession] =
    client.query("com.atproto.server.getSession", Vector.empty, session.accessJwt).flatMap { json =>
      for
        didText <- json.field("did").flatMap(_.asString).left.map(error => ClientError(error.message))
        did <- Did.parse(didText).left.map(error => ClientError(error.toString))
        handleText <- json.field("handle").flatMap(_.asString).left.map(error => ClientError(error.message))
        handle <- Handle.parse(handleText).left.map(error => ClientError(error.toString))
