...
object PkcePair:
  ...
  def generate(random: SecureRandom = SecureRandom()): PkcePair =
    ...
    fromVerifier(Base64Url.encode(bytes)).fold(error => throw IllegalStateException(error.message), identity)

  /** Validates a verifier and derives its mandatory S256 challenge. */
  def fromVerifier(verifier: String): Either[OAuthError, PkcePair] =
    if !Verifier.matches(verifier) then Left(OAuthError("PKCE verifier must contain 43-128 RFC 7636 unreserved characters"))
    else
...
object OAuthMetadata:
  /**
   * Parses PDS metadata and binds its `resource` field to the origin fetched.
   * HTTP is permitted only for explicit localhost development.
   */
  def parseProtectedResource(
    ...
    for
      ...
      _ <- require(normalizedResource == expected, "protected-resource metadata does not match the fetched PDS origin")
  ...
  def parseAuthorizationServer(
    ...
    for
      ...
      _ <- require(issuer == expected, "authorization-server issuer does not match the fetched origin")
      ...
      _ <- require(responseTypes.contains("code"), "authorization server must support response type code")
      grantTypes <- stringArrayField(json, "grant_types_supported")
      _ <- require(grantTypes.contains("authorization_code") && grantTypes.contains("refresh_token"), "authorization server must support authorization_code and refresh_token")
      ...
      _ <- require(authMethods.contains("none") && authMethods.contains("private_key_jwt"), "authorization server must support public and confidential clients")
      signing <- stringArrayField(json, "token_endpoint_auth_signing_alg_values_supported")
      _ <- require(signing.contains("ES256") && !signing.contains("none"), "authorization server must support ES256 client assertions and reject none")
      scopes <- stringArrayField(json, "scopes_supported")
      _ <- require(scopes.contains("atproto"), "authorization server must support the atproto scope")
      ...
      _ <- require(!requestUriRegistration.contains(false), "require_request_uri_registration may not be false")
    yield AuthorizationServerMetadata(issuer, authorization, token, par, scopes.toSet)

  private def endpointField(json: Json, name: String, allowLoopback: Boolean): Either[OAuthError, URI] =
    stringField(json, name).flatMap(parseUri(_, name)).flatMap(endpoint(_, allowLoopback, name))
  ...
  private def origin(uri: URI, allowLoopback: Boolean, label: String): Either[OAuthError, URI] =
    ...
    val defaultPort = (uri.getScheme == "https" && uri.getPort == 443) || (uri.getScheme == "http" && uri.getPort == 80)
    val valid = (uri.getScheme == "https" || loopback) && uri.getHost != null && uri.getRawUserInfo == null &&
      (path.isEmpty || path == "/") && uri.getRawQuery == null && uri.getRawFragment == null && !defaultPort
  ...
  private def endpoint(uri: URI, allowLoopback: Boolean, label: String): Either[OAuthError, URI] =
    val loopback = allowLoopback && uri.getScheme == "http" && uri.getHost == "localhost"
    val valid = (uri.getScheme == "https" || loopback) && uri.getHost != null && uri.getRawUserInfo == null &&
      uri.getRawFragment == null && uri.getRawQuery == null && Option(uri.getRawPath).exists(_.startsWith("/"))
    if valid then Right(uri) else Left(OAuthError(s"$label must be an absolute HTTPS endpoint without credentials or fragment"))
  ...
  private def stringField(json: Json, name: String): Either[OAuthError, String] =
    json.field(name).flatMap(_.asString).left.map(error => OAuthError(s"$name: ${error.message}"))

  private def stringArrayField(json: Json, name: String): Either[OAuthError, Vector[String]] =
    json.field(name).flatMap(_.asArray).left.map(error => OAuthError(s"$name: ${error.message}")).flatMap { values =>
      values.zipWithIndex.foldLeft[Either[OAuthError, Vector[String]]](Right(Vector.empty)) { case (result, (value, index)) =>
        for
          strings <- result
          string <- value.asString.left.map(error => OAuthError(s"$name[$index]: ${error.message}"))
        yield strings :+ string
      }
    }

  private def booleanField(json: Json, name: String): Either[OAuthError, Boolean] =
    json.field(name).flatMap(_.asBoolean).left.map(error => OAuthError(s"$name: ${error.message}"))
  ...
  private def optionalBooleanField(json: Json, name: String): Either[OAuthError, Option[Boolean]] =
    json.optionalField(name).left.map(error => OAuthError(error.message)).flatMap {
      ...
      case Some(value) => value.asBoolean.left.map(error => OAuthError(s"$name: ${error.message}")).map(Some.apply)
    }

  private def require(condition: Boolean, message: => String): Either[OAuthError, Unit] =
    Either.cond(condition, (), OAuthError(message))
...
object OAuthTokenResponse:
  ...
  def parse(json: Json, expectedSubject: Option[Did]): Either[OAuthError, OAuthTokenResponse] =
    for
      ...
      _ <- Either.cond(expectedSubject.forall(_ == subject), (), OAuthError("token subject does not match the account bound to the authorization flow"))
      access <- stringField(json, "access_token")
      tokenType <- stringField(json, "token_type")
      _ <- Either.cond(tokenType.equalsIgnoreCase("DPoP"), (), OAuthError("atproto access token must use token_type DPoP"))
      ...
      _ <- Either.cond(scopes.contains("atproto"), (), OAuthError("token scope must include atproto"))
  ...
  private def optionalStringField(json: Json, name: String): Either[OAuthError, Option[String]] =
    json.optionalField(name).left.map(error => OAuthError(error.message)).flatMap {
      case None => Right(None)
      case Some(value) => value.asString.left.map(error => OAuthError(s"$name: ${error.message}")).map(Some.apply)
    }

  private def longField(json: Json, name: String): Either[OAuthError, Long] =
    json.field(name).flatMap(_.asLong).left.map(error => OAuthError(s"$name: ${error.message}"))
