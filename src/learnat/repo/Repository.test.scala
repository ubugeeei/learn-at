...
object RepositoryTests:
  ...
  def run(): Unit =
    ...
    test("creates, updates, and deletes typed records with increasing revisions") {
      ...
      val updated = created.applyWrite(RepositoryWrite.Put(record(firstKey, "updated"))).toOption
        .get
      val withSecond = updated.applyWrite(RepositoryWrite.Create(record(secondKey, "two"))).toOption
        .get
      ...
      assert(
        RepositoryVerifier.verifyCar(deleted.exportCar.toOption.get, did, key.publicKey).isRight
      )
    }
    ...
    test("requires record maps with matching $type") {
      ...
      val repo = fresh(key)
      val missingType =
        RepositoryRecord(collection, firstKey, Ipld.obj("text" -> Ipld.Text("no type")))
      val wrongType = RepositoryRecord(
        collection,
        firstKey,
        Ipld.obj("$type" -> Ipld.Text("com.example.other"), "text" -> Ipld.Text("wrong"))
      )
    ...
    test("rejects a valid repository signed by a different key") {
      ...
      val repo = fresh(owner).applyWrite(RepositoryWrite.Create(record(firstKey, "one"))).toOption
        .get
    ...
    test("rejects missing record blocks and unreachable extras") {
      ...
      val withoutRecord = Car
        .write(parsed.copy(blocks = parsed.blocks.filterNot(_.cid == recordCid))).toOption.get
      ...
      assert(
        RepositoryVerifier.verifyCar(withExtra, did, key.publicKey, allowExtraBlocks = true).isRight
      )
    }
    ...
    test("exports the signed commit as the single CAR root and first block") {
      ...
      equal(
        repo.reference(collection, firstKey).map(_._1.toString),
        Some(s"at://${did.value}/${collection.value}/${firstKey.value}")
      )
    }
  ...
  private def record(key: RecordKey, text: String): RepositoryRecord = RepositoryRecord(
    collection,
    key,
    Ipld.obj(
