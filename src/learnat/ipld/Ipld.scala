...
object Cid:
  ...
  def create(codec: Long, content: Array[Byte]): Cid =
    ...
    val bytes = Varint.encode(1) ++ Varint.encode(codec) ++ Varint.encode(Sha256Code) ++
      Varint.encode(Sha256Length) ++ digest
  ...
  def parseBytes(bytes: Array[Byte]): Either[IpldError, Cid] = readPrefix(bytes, 0)
    .flatMap { (cid, next) =>
      if next == bytes.length then Right(cid)
      else Left(IpldError("trailing bytes after CID", Some(next)))
    }
  ...
  def readPrefix(bytes: Array[Byte], start: Int): Either[IpldError, (Cid, Int)] =
    for
      ...
      _ <- Either
        .cond(version == 1, (), IpldError(s"unsupported CID version: $version", Some(start)))
      ...
      _ <- Either.cond(
        hashCode == Sha256Code,
        (),
        IpldError(s"unsupported multihash code: $hashCode", Some(afterCodec))
      )
      ...
      _ <- Either.cond(
        length == Sha256Length,
        (),
        IpldError(s"unsupported SHA-256 digest length: $length", Some(afterHash))
      )
      end <-
        ...
        Either.cond(
          ...
          candidate.toInt,
          IpldError("truncated CID digest", Some(afterLength))
        )
...
object Varint:
  ...
  def decode(bytes: Array[Byte], start: Int): Either[IpldError, (Long, Int)] =
    ...
    while index < bytes.length && !complete && shift <= 63 do
      ...
      if shift == 63 && (current & 0xfe) != 0 then
        return Left(IpldError("varint overflows 64 bits", Some(index)))
    ...
    else if index - start > 1 && (bytes(index - 1) & 0x7f) == 0 then
      Left(IpldError("non-minimal varint", Some(start)))
  ...
  def decode(value: String): Either[IpldError, Array[Byte]] =
    if value.isEmpty then Left(IpldError("empty base32 value"))
      ...
      while index < value.length do
        ...
        if digit < 0 then
          return Left(
            IpldError(s"invalid lower-case base32 character: ${value.charAt(index)}", Some(index))
          )
...
object DagCbor:
  ...
  def decode(bytes: Array[Byte], limits: Limits = Limits()): Either[IpldError, Ipld] =
    if bytes.length > limits.maxBytes then
      Left(IpldError(s"DAG-CBOR input exceeds ${limits.maxBytes} bytes"))
    else
      val decoder = Decoder(bytes, limits)
      decoder.value(1).flatMap { decoded =>
        if decoder.atEnd then Right(decoded)
        else Left(IpldError("trailing bytes after DAG-CBOR value", Some(decoder.offset)))
      }

  /**
   * Decodes a concatenated sequence of canonical DAG-CBOR values. Event-stream frames use this form
   * for a header followed by a body.
   */
  def decodeSequence(
      bytes: Array[Byte],
    ...
    if bytes.length > limits.maxBytes then
      ...
      while !decoder.atEnd && failure.isEmpty do
        if count >= maxItems then
          failure =
            Some(IpldError(s"DAG-CBOR sequence exceeds $maxItems items", Some(decoder.offset)))
        else
  ...
  private def encodeValue(value: Ipld, out: ByteArrayOutputStream): Either[IpldError, Unit] =
    value match
      case Ipld.Null =>
        out.write(0xf6)
        Right(())
      ...
      case Ipld.List(values) =>
        ...
        values.foldLeft[Either[IpldError, Unit]](Right(
          ()
        ))((result, item) => result.flatMap(_ => encodeValue(item, out)))
  ...
  private def encodeMap(
      fields: Vector[(String, Ipld)],
      out: ByteArrayOutputStream
  ): Either[IpldError, Unit] =
    val duplicates = fields.groupMapReduce(_._1)(_ => 1)(_ + _)
      .collect { case (key, count) if count > 1 => key }
    if duplicates.nonEmpty then
      Left(IpldError(s"duplicate DAG-CBOR map keys: ${duplicates.mkString(", ")}"))
    else
  ...
  private def compareKeys(left: Array[Byte], right: Array[Byte]): Int =
    ...
    if length != 0 then length else Arrays.compareUnsigned(left, right)

  final private class Decoder(bytes: Array[Byte], limits: Limits):
    var offset: Int = 0
      ...
      if depth > limits.maxDepth then fail(s"DAG-CBOR nesting exceeds ${limits.maxDepth}")
        ...
        major match
          case 0 => unsigned(additional, initialOffset).flatMap(number =>
              if number <= Long.MaxValue then Right(Ipld.Integer(number))
              else fail("positive integer exceeds signed 64-bit range")
            )
          case 1 => unsigned(additional, initialOffset).flatMap(number =>
              if number <= Long.MaxValue then Right(Ipld.Integer(-1L - number))
              else fail("negative integer exceeds signed 64-bit range")
            )
          case 2 => byteString(additional, initialOffset)
              .map(value => Ipld.Bytes(ByteString(value)))
    ...
    private def sequence(additional: Int, initialOffset: Int, depth: Int): Either[IpldError, Ipld] =
      length(additional, initialOffset).flatMap { count =>
        Vector.fill(count)(()).foldLeft[Either[IpldError, Vector[Ipld]]](Right(Vector.empty)) {
          (result, _) => result.flatMap(values => value(depth + 1).map(values :+ _))
        }.map(Ipld.List.apply)
      }
    ...
    private def map(additional: Int, initialOffset: Int, depth: Int): Either[IpldError, Ipld] =
      length(additional, initialOffset).flatMap { count =>
        ...
        Vector.fill(count)(())
          .foldLeft[Either[IpldError, Vector[(String, Ipld)]]](Right(Vector.empty)) { (result, _) =>
            result.flatMap { fields =>
              val keyStart = offset
              if keyStart >= bytes.length || ((bytes(keyStart) & 0xff) >>> 5) != 3 then
                fail("DAG-CBOR map keys must be strings")
              else
                val initial = readByte()
                text(initial & 31, keyStart).flatMap { key =>
                  val keyBytes = key.getBytes(StandardCharsets.UTF_8)
                  if previous.exists(value => compareKeys(value, keyBytes) >= 0) then
                    fail("DAG-CBOR map keys are duplicate or not in canonical order")
                  else
    ...
    private def link(additional: Int, initialOffset: Int, depth: Int): Either[IpldError, Ipld] =
      ...
      unsigned(additional, initialOffset).flatMap { tag =>
        ...
        else if offset >= bytes.length || ((bytes(offset) & 0xff) >>> 5) != 2 then
          fail("CID tag must contain a byte string")
        else
          val bytesOffset = offset
          val initial = readByte()
          byteString(initial & 31, bytesOffset).flatMap { value =>
            if value.isEmpty || value.head != 0 then
              fail("CID byte string must start with identity multibase byte 0")
    ...
    private def unsigned(additional: Int, initialOffset: Int): Either[IpldError, Long] =
      additional match
    ...
    private def minimal(value: Long, minimum: Long, initialOffset: Int): Either[IpldError, Long] =
      if value < minimum then
        Left(IpldError("non-minimal CBOR integer or length encoding", Some(initialOffset)))
      else Right(value)
    ...
    private def readUnsigned(count: Int): Either[IpldError, Long] =
      if count > bytes.length - offset then fail("truncated CBOR integer")
        ...
        while index < count do
          ...
          index += 1
        if value < 0 then fail("unsigned CBOR value exceeds signed implementation range")
        else Right(value)
    ...
    private def fail[A](message: String): Left[IpldError, A] =
      Left(IpldError(message, Some(offset)))
