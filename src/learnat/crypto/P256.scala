  ...
  def decode(value: String): Either[CryptoError, Array[Byte]] =
    if value.isEmpty then Right(Array.emptyByteArray)
      ...
      while index < value.length && failure.isEmpty do
        ...
        if digit < 0 then
          failure = Some(CryptoError(s"invalid base58btc character '${value
      ...
      failure.toLeft {
        ...
        val magnitude =
          if signed.length > 1 && signed.head == 0 then signed.tail
          else if number == BigInteger.ZERO then Array.emptyByteArray
          else signed
 ...
 *
 * The wire representation is `z` multibase, the two-byte `p256-pub` multicodec prefix (`0x80
 * 0x24`), and a 33-byte SEC1 compressed public key.
 */
final class P256PublicKey private[crypto] (private val key: ECPublicKey):
  ...
  /**
   * Verifies an atproto compact ECDSA signature. DER signatures, out-of-range scalars, and
   * malleable high-S signatures fail.
   */
  def verify(message: Array[Byte], compactSignature: Array[Byte]): Boolean = P256
    .verify(key, message, compactSignature)
  ...
  def generate(): Either[CryptoError, P256KeyPair] =
    try
      ...
      pair.getPublic match
        case public: ECPublicKey =>
          Right(new P256KeyPair(pair.getPrivate, new P256PublicKey(public)))
        case _ => Left(CryptoError("JCA provider did not return an EC public key"))
    catch
      case error: Exception => Left(CryptoError(s"P-256 generation failed: ${error.getMessage}"))
      ...
      privateKey <-
        try Right(KeyFactory.getInstance("EC").generatePrivate(PKCS8EncodedKeySpec(pkcs8)))
          ...
          case error: Exception =>
            Left(CryptoError(s"invalid PKCS#8 P-256 private key: ${error.getMessage}"))
...
object P256:
  ...
  def publicKeyFromMultikey(value: String): Either[CryptoError, P256PublicKey] =
    if !value.startsWith("z") then
      Left(CryptoError("P-256 multikey must use base58btc multibase prefix 'z'"))
    else
      Base58Btc.decode(value.drop(1)).flatMap { bytes =>
        if bytes.length != MulticodecPrefix.length + 33 then
          Left(CryptoError(s"P-256 multikey must contain 35 bytes, found ${bytes.length}"))
        else if !Arrays.equals(bytes.take(2), MulticodecPrefix) then
          Left(CryptoError("multikey is not prefixed with the p256-pub multicodec"))
  ...
  def publicKeyFromCoordinates(x: Array[Byte], y: Array[Byte]): Either[CryptoError, P256PublicKey] =
    if x.length != CoordinateLength || y.length != CoordinateLength then
      ...
      decompress(Array(prefix) ++ x).flatMap { publicKey =>
        if MessageDigest.isEqual(coordinate(publicKey.getW.getAffineY), y) then
          Right(new P256PublicKey(publicKey))
  ...
  private def decompress(bytes: Array[Byte]): Either[CryptoError, ECPublicKey] =
    if bytes.length != 33 || (bytes.head != 0x02.toByte && bytes.head != 0x03.toByte) then
      ...
      try
        ...
        if x.compareTo(FieldPrime) >= 0 then
          Left(CryptoError("P-256 x coordinate is outside the field"))
        else
          val ySquared = x.modPow(BigInteger.valueOf(3), FieldPrime)
            .subtract(x.multiply(BigInteger.valueOf(3))).add(CurveB).mod(FieldPrime)
          val candidate = ySquared.modPow(FieldPrime.add(BigInteger.ONE).shiftRight(2), FieldPrime)
          if candidate.multiply(candidate).mod(FieldPrime) != ySquared then
            Left(CryptoError("compressed key is not a P-256 curve point"))
          else
            val odd = bytes.head == 0x03.toByte
            val y =
              if candidate.testBit(0) == odd then candidate else FieldPrime.subtract(candidate)
        ...
        case error: Exception =>
          Left(CryptoError(s"invalid compressed P-256 key: ${error.getMessage}"))
  ...
  private[crypto] def verify(key: PublicKey, message: Array[Byte], compact: Array[Byte]): Boolean =
    if compact.length != 64 then false
      ...
      if r.signum() <= 0 || r.compareTo(Order) >= 0 || s.signum() <= 0 || s.compareTo(HalfOrder) > 0
      then false
      else
        try
  ...
  private[crypto] def derToCompact(der: Array[Byte]): Either[CryptoError, Array[Byte]] = parseDer(
    der
  ).flatMap { (r, originalS) =>
    if r.signum() <= 0 || r.compareTo(Order) >= 0 || originalS.signum() <= 0 ||
      originalS.compareTo(Order) >= 0
    then Left(CryptoError("ECDSA signature scalar is outside the P-256 order"))
    else
  ...
  private def parseDer(der: Array[Byte]): Either[CryptoError, (BigInteger, BigInteger)] =
    try
      var offset = 0
      if read(der, offset) != 0x30 then
        return Left(CryptoError("ECDSA signature is not a DER sequence"))
      offset += 1
      val (sequenceLength, afterSequenceLength) = readLength(der, offset)
      offset = afterSequenceLength
      if offset + sequenceLength != der.length then
        return Left(CryptoError("invalid DER sequence length"))
      val (r, afterR) = readInteger(der, offset)
      val (s, afterS) = readInteger(der, afterR)
      if afterS != der.length then Left(CryptoError("trailing bytes in DER ECDSA signature"))
      else Right(r -> s)
  ...
  private def readInteger(bytes: Array[Byte], start: Int): (BigInteger, Int) =
    ...
    if length < 1 || valueStart + length > bytes.length then
      throw IllegalArgumentException("truncated DER integer")
    ...
    if encoded.length > 1 && encoded.head == 0 && (encoded(1) & 0x80) == 0 then
      throw IllegalArgumentException("non-minimal DER integer")
  ...
  private def readLength(bytes: Array[Byte], start: Int): (Int, Int) =
    ...
    if first < 128 then first -> (start + 1)
      ...
      if count < 1 || count > 2 || start + 1 + count > bytes.length then
        throw IllegalArgumentException("unsupported DER length")
      var value = 0
      var index = 0
  ...
  private def read(bytes: Array[Byte], offset: Int): Int =
    if offset >= bytes.length then throw IllegalArgumentException("truncated DER ECDSA signature")
    else bytes(offset) & 0xff
  ...
  private def fixedUnsigned(value: BigInteger, length: Int): Array[Byte] =
    ...
    if magnitude.length > length then
      throw IllegalArgumentException(s"integer does not fit in $length bytes")
    Array.fill[Byte](length - magnitude.length)(0) ++ magnitude

  private[crypto] def coordinate(value: BigInteger): Array[Byte] =
    fixedUnsigned(value, CoordinateLength)
