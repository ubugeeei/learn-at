...
object AuthorizationRequest:
  ...
  def create(
    ...
    for
      ...
      _ <- Either.cond(scopes.nonEmpty && scopes.contains("atproto"), (), OAuthError("OAuth scopes must include atproto"))
      _ <- Either.cond(scopes.distinct.length == scopes.length, (), OAuthError("OAuth scopes must not contain duplicates"))
      _ <- Either.cond(scopes.forall(validScope), (), OAuthError("OAuth scope contains whitespace or a control character"))
  ...
  private def validateClientId(value: URI): Either[OAuthError, Unit] =
    ...
    val localhost = value.getScheme == "http" && value.getHost == "localhost" && value.getPort == -1 &&
      value.getRawUserInfo == null && value.getRawFragment == null &&
      (Option(value.getRawPath).forall(_.isEmpty) || value.getRawPath == "/")
    Either.cond(secure || localhost, (), OAuthError("client_id must be an HTTPS metadata URL without a port, or the http://localhost development client"))
  ...
  private def validateRedirect(value: URI): Either[OAuthError, Unit] =
    ...
    val loopback = value.getScheme == "http" && Set("127.0.0.1", "[::1]", "::1").contains(value.getHost)
    val native = value.getScheme != null && value.getScheme != "http" && value.getScheme != "https" &&
      Option(value.getRawSchemeSpecificPart).exists(_.startsWith("/"))
    Either.cond(absolute && (secureWeb || loopback || native), (), OAuthError("redirect_uri is not a permitted web, loopback, or native callback URI"))

  private def validScope(value: String): Boolean =
    value.nonEmpty && value.forall(character => character > ' ' && character != 0x7f.toChar)
...
final case class PushedAuthorizationResponse(requestUri: String, expiresInSeconds: Long):
  ...
  def browserUri(metadata: AuthorizationServerMetadata, clientId: URI): URI =
    val query = OAuthForm.query(Vector("client_id" -> clientId.toString, "request_uri" -> requestUri))
...
object PushedAuthorizationResponse:
  ...
  def parse(json: learnat.json.Json): Either[OAuthError, PushedAuthorizationResponse] =
    for
      requestUri <- json.field("request_uri").flatMap(_.asString).left.map(error => OAuthError(s"request_uri: ${error.message}"))
      _ <- Either.cond(requestUri.nonEmpty, (), OAuthError("request_uri must not be empty"))
      expires <- json.field("expires_in").flatMap(_.asLong).left.map(error => OAuthError(s"expires_in: ${error.message}"))
...
object AuthorizationCallback:
  ...
  def parse(uri: URI, expectedState: OAuthState, expectedIssuer: URI): Either[OAuthError, AuthorizationCallback] =
    for
      ...
      _ <- Either.cond(issuer == expectedIssuer.toString, (), OAuthError("OAuth callback issuer does not match"))
      result <- (parameters.get("code"), parameters.get("error")) match
        ...
        case (None, Some(error)) if error.nonEmpty => Right(AuthorizationCallback.Denied(error, parameters.get("error_description")))
        case _ => Left(OAuthError("OAuth callback must contain exactly one non-empty code or error"))
    yield result

  private def parseQuery(uri: URI): Either[OAuthError, Map[String, String]] =
    Option(uri.getRawQuery).toRight(OAuthError("OAuth callback has no query string")).flatMap { query =>
      query.split("&", -1).toVector.foldLeft[Either[OAuthError, Map[String, String]]](Right(Map.empty)) { (result, part) =>
        result.flatMap { parameters =>
          ...
          for
            ...
            _ <- Either.cond(name.nonEmpty, (), OAuthError("OAuth callback contains an empty parameter name"))
            _ <- Either.cond(!parameters.contains(name), (), OAuthError(s"OAuth callback repeats parameter $name"))
  ...
  private def decode(value: String): Either[OAuthError, String] =
    ...
    catch case _: IllegalArgumentException => Left(OAuthError("OAuth callback contains invalid percent encoding"))
  ...
  private def constant(actual: String, expected: String, message: String): Either[OAuthError, Unit] =
    val equal = MessageDigest.isEqual(actual.getBytes(StandardCharsets.UTF_8), expected.getBytes(StandardCharsets.UTF_8))
...
object TokenRequest:
  ...
  def authorizationCode(code: String, request: AuthorizationRequest): Either[OAuthError, TokenRequest] =
    if code.isEmpty then Left(OAuthError("authorization code must not be empty"))
    else Right(TokenRequest(Vector(
...
private object OAuthForm:
  def encode(fields: Vector[(String, String)]): Array[Byte] =
    fields.map { (name, value) => s"${form(name)}=${form(value)}" }.mkString("&").getBytes(StandardCharsets.UTF_8)

  def query(fields: Vector[(String, String)]): String =
    fields.map { (name, value) => s"${queryComponent(name)}=${queryComponent(value)}" }.mkString("&")
